...
- **Description:** There is an ongoing inconsistency in the `@google/genai` SDK regarding how to pass the `displayName` during file upload.
  - One implementation requires a top-level `displayName` property.
  - Another implementation requires `displayName` to be nested within a `fileMetadata` object.
...
- **Current Workaround & Instruction:**
  ...
  - We will continue to use the local storage cache to manage display names after upload, as the API does not reliably return the provided name.
